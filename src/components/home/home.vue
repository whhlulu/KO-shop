<template>
    <div class="home">
        ...
        <limited-hd v-if="$store.state.home_data.promotions" :text="text[0]" :toLink="'/latestProm'"></limited-hd>
        <limited-dd v-if="$store.state.home_data.promotions" :data="$store.state.home_data.promotions" :banner="$store.state.home_data.promotions_img"></limited-dd>
        <limited-activity v-if="$store.state.home_data.poopClear" :data="$store.state.home_data.poopClear" :endTime="Number($store.state.home_data.endtime)"></limited-activity>
        <div class="limit-banner"v-if="$store.state.home_data.poopClear" v-for="item in $store.state.home_data.poopClear_img" :key="item.id">
            <img v-lazy="URL + item.pic_url">
        </div>
        <limited-hd v-if="$store.state.home_data.brand" :text="text[1]" :toLink="'/brandList'"></limited-hd>
        <brand-mus v-if="$store.state.home_data.brand" :data="$store.state.home_data.brand" :banner="$store.state.home_data.brand_img"></brand-mus>
        <limited-hd v-if="$store.state.home_data.integral" :text="text[2]" :toLink="'/IntegralMall'"></limited-hd>
        <late-mall v-if="$store.state.home_data.integral" :banner="$store.state.home_data.integral_top_img" :status="2" :data="$store.state.home_data.integral"></late-mall>
        <div class="limit-banner" v-if="$store.state.home_data.integral"><img v-for="item in $store.state.home_data.integral_foot_img" :key="item.id" v-lazy="URL + item.pic_url"></div>
        <div class="goods-class"  v-for="(item,index) in classGoods">
            <limited-hd :text="item.class_name" :toLink="item.id" :status="'mobile'"></limited-hd>
            <hot-goods :data="item.goods"></hot-goods>
        </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            getClassGoods(){
                this.axios({
                url:API_URL+'Home/Index/getClassGoods',
                method:'get'
            }).then((res) => {
                if(res.data.status == 1){
                     this.classGoods = res.data.data;
                }
            }).catch((err) => {
                console.log(err)
            });
