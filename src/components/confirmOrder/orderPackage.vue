	...
	<div class="order-wrap">
		...
		<div class="price-set-wrap"  >
			<div class="total price clearfix">
				...
				<span class="fr">￥{{pre_preferential_price|keepTwoNum}}</span>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//获取数据
			getData(){
                this.axios.post(this.$httpConfig.purchaseImmediately,qs.stringify({
		...
		// 提交订单
			...
			tolink(to,index) {
				...
				sessionStorage.setItem('set_meal_id',this.$route.params.set_meal_id);
				if(to == 'address') {
					this.$router.push({
						name: to,
						params:{
